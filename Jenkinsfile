pipeline {
    ...
    environment {
      ORG               = 'activiti'
      APP_NAME          = 'activiti-cloud-modeling-dependencies'
    }
    stages {
    ...
    post {
        failure {
           slackSend(
             ...
             color: "danger",
             message: "	activiti-cloud-modeling-dependencies branch=$BRANCH_NAME is failed http://jenkins.jx.35.242.205.159.nip.io/job/Activiti/job/activiti-cloud-modeling-dependencies"
           )
        } 
        always {
            cleanWs()
